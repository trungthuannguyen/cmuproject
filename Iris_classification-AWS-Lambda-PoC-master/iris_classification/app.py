...
# Function to read result data from file
def read_result_data():
    # Replace with the path to your result data file
    result_file_path = "src/dataset_result.csv"
...
# Read and display result data on a chart
result_data = read_result_data()
x = np.arange(len(slider_labels))

fig, ax = plt.subplots(figsize=(10, 6))
colors = ['r', 'g', 'b', 'c', 'm', 'y', 'k', 'lime', 'orange', 'purple', 'pink', 'skyblue', 'gold']
ax.bar(x, result_data.iloc[0], align='center', alpha=0.5, color=colors)
ax.set_xticks(x)
ax.set_xticklabels([])
ax.set_ylabel("Value")
...
for i, v in enumerate(result_data.iloc[0]):
